...
class Entangld extends EventEmitter {
    ...
    constructor({
        ...
        this._deref_mode = deref_mode == undefined ? false : deref_mode;
    }
     ...
     */
    async call_rpc(path, args=[]) {
        return this.get(path,args).then((args) => {
            if ( !Array.isArray(args) || args.length != 2) throw new EntangldError(`Endpoint is not configured for RPC: '${path}'`);
