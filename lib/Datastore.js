...
const EntangldError = require("./Error.js");
const EntangldMessage = require("./Message.js");
const EntangldSubscription = require("./Subscription.js");
const { dereferenced_copy, extract_from_path, partial_copy, is_beneath } = require("./utils.js");
 ...
 */
class EntangldDatastore extends EventEmitter {
     ...
     *
     * @readonly
     * @return {EntangldSubscription[]} array of Subscriptions associated with this object
     */
    get subscriptions() {
     ...
     * EntangldMessage object and obj is the object provided by attach().
     *
     ...
     * callback.
     *
     * @param {EntangldMessage} msg the message to process.
    ...
    receive(msg, obj) {
        ...
        if (msg.type == "set" || msg.type == "push") {
            ...
            this.get(msg.path, msg.value).then((val) => {
                ...
                const response = EntangldMessage.value(msg,val);
            ...
            if (count === 0) {
                ...
                const response = EntangldMessage.unsubscribe(msg.uuid);
                this._transmit(response, obj);
            }
            ...
            this._subscribe(msg.path, (path, val) => {
                ...
                const response = EntangldMessage.event(path,val,msg.uuid);
                this._transmit(response, obj);
    ...
    set(path, data, operation_type="set", params={}) {
        ...
        if (obj === undefined) {
            ...
            const msg = EntangldMessage.setpush({
                type : operation_type,
                tree : tree,
                value : data,
                params : params
    ...
    get(path, params) {
        ...
        const msg = EntangldMessage.get(tree, params);
     ...
     * @param {string} path the path to watch.
     * @param {function} func the callback - will be of the form (path, value).
     * @param {(EntangldDatastore|null)} [upstream=null] the Engangld next upstream in the path.
    ...
    _subscribe(path, func, upstream = null, uuid = null, every = null) {
        ...
        let new_sub = new EntangldSubscription({
            path : path,
            downstream : obj,
            upstream : upstream,
            uuid : uuid,
        ...
        if (new_sub.has_downstream) {
            const msg = EntangldMessage.subscribe(tree, uuid, every);
            this._transmit(msg, obj);
    ...
    _unsubscribe(subscriptions) {
        ...
        subscriptions.forEach((s) => {
            if (!s.has_downstream) { // emit unsubscription
                ...
                const msg = EntangldMessage.unsubscribe(s.uuid);
...
module.exports = exports = EntangldDatastore;
